...
Bucket_3_port = 'COM49'
baud_rate = 9600
b1Serial = serial.Serial(Bucket_3_port, baud_rate)

directoryBase = r"C:\Users\Dan's Loaner\Documents\Compost Monitor\Test Data\{}\Bucket 3\CH4".format(time.strftime("%m-%d-%Y"))
pathlib.Path(directoryBase).mkdir(parents=True, exist_ok=True)
logFileCH4  = '{}\\CH4_Bucket_3_{}_{}_log.bin'.format(directoryBase, time.strftime("%m-%d-%Y"), time.strftime("%H;%M;%S"))
csvCH4 = '{}\\CH4_Bucket_3_{}_{}.csv'.format(directoryBase, time.strftime("%m-%d-%Y"), time.strftime("%H;%M;%S"))
