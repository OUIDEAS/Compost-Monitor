...
while 1:
    if PMQSerial.is_open:
        ...
        if (fileCount ==1 and count >= 1 and PMQ_inbyte == b'\n'):
            ...
            # print(PMQ_DataList)
            PMQ_DataDict = {'Date_Time': PMQ_DataList[0], 'BME_Humidity': PMQ_DataList[1],
                            'BME_Pressure': PMQ_DataList[2], 'BME_Temp': PMQ_DataList[3], 'Sensor': 'PMQ',
        ...
        print(f'PMQ {args.containernumber} disconnected, waiting for reconnect...')
        while not PMQSerial.is_open:
            try:
                PMQSerial.open()
                print(f'Reconnected to PMQ {args.containernumber}')
